...
<div class="users view large-9 medium-8 columns content">
    ...
    <table class="vertical-table">
        ...
        <tr>
            <th scope="row"><?= __('Logins Id') ?></th>
            <td><?= $this->Number->format($user->logins_id) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Logins Roles Id') ?></th>
            <td><?= $this->Number->format($user->logins_roles_id) ?></td>
        </tr>
    </table>
    <div class="related">
        <h4><?= __('Related Logins') ?></h4>
        <?php if (!empty($user->logins)): ?>
        <table cellpadding="0" cellspacing="0">
            ...
            <?php foreach ($user->logins as $logins): ?>
            <tr>
                <td><?= h($logins->id) ?></td>
                <td><?= h($logins->username) ?></td>
                <td><?= h($logins->password) ?></td>
                <td><?= h($logins->user_id) ?></td>
                <td><?= h($logins->role_id) ?></td>
                <td><?= h($logins->roles_id) ?></td>
                <td class="actions">
